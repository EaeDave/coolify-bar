...
test "$(jq -r '.recent[] | select(.id=="fin-stub-2") | .statusKind' <<<"$payload")" = "success"
test "$(jq -r '.barWidget.defaults.notificationsEnabled' "$root/manifest.json")" = "true"
test "$(jq -r '.barWidget.schema[] | select(.key=="notificationsEnabled") | .defaultValue' "$root/manifest.json")" = "true"
test "$(jq -r '.barWidget.defaults.notificationSoundEnabled' "$root/manifest.json")" = "true"
test "$(jq -r '.barWidget.schema[] | select(.key=="notificationSoundEnabled") | .defaultValue' "$root/manifest.json")" = "true"
test -s "$root/coolify-notification.wav"
